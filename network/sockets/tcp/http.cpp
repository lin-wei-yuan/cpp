...
void simple_http::request(const std::string& address)
{
...
std::vector<std::string> simple_http::split(const std::string& value, char delim)
{
...
void simple_http::add_headers(const std::string& key, const std::string& value)
{
    for(auto &p : m_request_headers)
    {
        if (p.first == key)
...
std::string simple_http::get_header(const std::string& key) const
{
...
std::string simple_http::trim(const std::string& value)
{
    const std::string whitespace = " \t\f\v\n\r";
    std::string result(value);
    int start = result.find_first_not_of(whitespace);
    int end = result.find_last_not_of(whitespace);
    result.erase(0, start);
    result.erase((end - start) + 1);
    return result;
}
...
void simple_http::set_method(const std::string& method)
{
    m_request_method = method;
}

void simple_http::set_uri(const std::string& uri)
{
    m_request_uri = uri;
}
